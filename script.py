...
def result():
    if request.method == 'POST':
        ...
        to_predict_list.pop(6)
        to_predict_list = list(map(int, to_predict_list))
        if str(request.form.get("pet")) == "1":
            result = ValuePredictor2(to_predict_list)  
            parte_entera = math.trunc(result)
            return render_template("result2.html", prediction = parte_entera, res_sup=res_sup, res_const=res_const, res_rooms=res_rooms, res_brms=res_brms, res_prk=res_prk, res_ant=res_ant)
